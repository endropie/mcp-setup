  ...
  implements PromptProtocol
{
  ...
  protected async validateArgs(args: Record<string, unknown>): Promise<TArgs> {
    return this.schema instanceof z.ZodObject
      ? this.schema.parse(args) as TArgs
      : args as TArgs;
  }
    ...
    const zodSchema = z.object(
      Object.fromEntries(
        Object.entries(this.schema).map(([key, schema]) => {
          if (this.isZodObjectType(schema.type)) {
            return [key, schema.type];
          }
          
          return [key, jsonSchemaToZod(schema.type)]
        })
      )
    );
