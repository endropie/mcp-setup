...
export async function validateMCPProject() {
  try {
    ...
    if (
      !package_json.dependencies?.["mcp-setup"] &&
      !package_json.devDependencies?.["mcp-setup"]
    ) {
      throw new Error(
        "This directory is not an MCP project (mcp-setup not found in dependencies or devDependencies)"
      );
    }
  } catch (error) {
