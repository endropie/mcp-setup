...
import { HttpTransport } from '../transports/http/server.js';
...
import { HttpTransportConfig, DEFAULT_HTTP_CONFIG } from '../transports/http/types.js';
...
export interface TransportConfig {
  options?: SSETransportConfig | HttpTransportConfig;
  auth?: AuthConfig;
}
...
export class MCPServer {
  ...
  private transportConfig: Record<TransportType, TransportConfig>;
    ...
    this.transportConfig = {
      ...
      sse: {
        options: {
          ...
          ...((config.transport && config.transport.sse && config.transport.sse.options) || {}),
      ...
      http: {
        options: {
          ...DEFAULT_HTTP_CONFIG,
          cors: DEFAULT_CORS_CONFIG,
          ...((config.transport && config.transport.http && config.transport.http.options) || {}),
        },
        auth: undefined,
      } as TransportConfig,
    };
  ...
  private createTransport(): BaseTransport {
    ...
    const options = this.transportConfig[type].options || {};
    const authConfig = this.transportConfig[type].auth ?? (options as any).auth;
    ...
    switch (type) {
      ...
      case 'http': {
        const httpConfig: HttpTransportConfig = {
          ...DEFAULT_HTTP_CONFIG,
          ...(options as HttpTransportConfig),
          cors: {
            ...DEFAULT_CORS_CONFIG,
            ...((options as HttpTransportConfig).cors || {}),
          },
          auth: authConfig,
        };
        logger.debug(`Creating HttpTransport. response mode: ${httpConfig.responseMode}`);
      ...
      default:
        if (type !== 'stdio') {
          logger.warn(`Unsupported type '${type}', defaulting to stdio.`);
        }
